...
export default function SlaSidebarPanel() {
    ...
    return (
        <div className="border-t border-border px-4 py-3">
            <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">SLA — {sla.policy.name}</p>

            {sla.is_paused && <p className="mb-2 text-xs text-muted-foreground italic">Clock paused — waiting for customer response.</p>}
                ...
                <div className="flex items-center justify-between text-sm">
                    ...
                    <div className="flex items-center gap-2">
                        {sla.first_response_status === 'pending' && (
                            <span className="text-xs text-muted-foreground">{formatRemaining(sla.first_response_remaining_minutes)}</span>
                        )}
                ...
                <div className="flex items-center justify-between text-sm">
                    ...
                    <div className="flex items-center gap-2">
                        {sla.resolution_status === 'pending' && (
                            <span className="text-xs text-muted-foreground">{formatRemaining(sla.resolution_remaining_minutes)}</span>
                        )}
