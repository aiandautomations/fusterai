...
interface MailboxStat  { mailbox_id: number; count: number; mailbox?: { id: number; name: string } }
interface ChannelStat  { channel_type: string; count: number }
interface PriorityStat { priority: string; count: number }
interface TrendStat    { date: string; count: number }
interface AgentStat    { id: number; name: string; email: string; resolved_count: number }
...
interface Props { stats: Stats; days: number }

const trendChartConfig = {
  count: { label: 'Conversations', color: 'var(--primary)' },
} satisfies ChartConfig

function StatCard({ label, value, tone }: { label: string; value: number | string; tone?: 'default' | 'info' | 'warning' | 'success' | 'primary' }) {
      ...
      <div className="w-full px-6 py-8 space-y-10">
        ...
        <section className="space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
            ...
            <StatCard label="Avg Resolution" value={stats.avg_resolution_hours > 0 ? `${stats.avg_resolution_hours.toFixed(1)}h` : '—'} tone="primary" />
          </div>
        </section>
                  ...
                  return stats.by_channel.map(item => (
                    <li key={item.channel_type} className="flex items-center gap-4 px-5 py-3">
                      ...
                      <div className="flex-1 h-1.5 rounded-full bg-muted overflow-hidden">
                        <div className="h-full rounded-full bg-primary/50 transition-all" style={{ width: `${(item.count / max) * 100}%` }} />
                      </div>
                ...
                {(() => {
                  ...
                  return stats.by_mailbox.map(item => (
                    <li key={item.mailbox_id} className="flex items-center gap-4 px-5 py-3">
                      <span className="text-sm font-medium flex-1 min-w-0 truncate">{item.mailbox?.name ?? `Mailbox #${item.mailbox_id}`}</span>
                      <div className="w-32 h-1.5 rounded-full bg-muted overflow-hidden shrink-0">
                        <div className="h-full rounded-full bg-primary/50 transition-all" style={{ width: `${(item.count / max) * 100}%` }} />
                      </div>
                      <span className="text-sm tabular-nums text-muted-foreground w-24 text-right shrink-0">{item.count} conversations</span>
                    </li>
                  ))
                })()}
              </ul>
