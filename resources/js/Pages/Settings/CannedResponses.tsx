...
interface CannedResponse { id: number; name: string; content: string; mailbox_id: number | null; mailbox?: { id: number; name: string } | null }
interface Props          { responses: CannedResponse[]; mailboxes: Mailbox[] }

function MailboxSelect({ value, onChange, mailboxes, includeAll }: {
    value: number | null | '';
            ...
            <form onSubmit={save} className="py-4 space-y-3 border-t first:border-t-0">
                ...
                <div className="space-y-1">
                    ...
                    <MailboxSelect
                        ...
                        onChange={v => setData('mailbox_id', v)}
                        mailboxes={mailboxes}
                    />
                </div>
...
function AddForm({ mailboxes }: { mailboxes: Mailbox[] }) {
    ...
    const { data, setData, post, processing, reset } = useForm<{ name: string; content: string; mailbox_id: number | null }>({
        name: '', content: '', mailbox_id: null,
    })

    function submit(e: React.FormEvent) {
        e.preventDefault()
        post('/settings/canned-responses', { onSuccess: () => { reset(); setOpen(false) } })
    }
            ...
            <div className="w-full px-6 py-8 space-y-6">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                        Save common replies and insert them by typing <kbd className="px-1.5 py-0.5 rounded border text-xs font-mono">/</kbd> in the reply box.
                        Workspace-wide responses appear in all mailboxes; mailbox-specific ones appear only in that mailbox.
                    </p>
                </div>
                    ...
                    {mailboxes.length > 0 && (
                        <div className="flex items-center gap-2">
                            ...
                            <select
                                ...
                                onChange={e => setFilterMailbox(e.target.value === '' ? '' : e.target.value === 'null' ? null : Number(e.target.value))}
                ...
                <Card>
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm flex items-center gap-2">
                            ...
                            Workspace-wide
                            <Badge variant="secondary" className="ml-auto">{workspaceWide.length}</Badge>
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                    ...
                    <Card>
                        <CardHeader className="pb-2">
                            <CardTitle className="text-sm flex items-center gap-2">
                                ...
                                Mailbox-specific
                                <Badge variant="secondary" className="ml-auto">{mailboxSpecific.length}</Badge>
                            </CardTitle>
                        </CardHeader>
                        <CardContent>
                            {mailboxSpecific.map(r => <CannedResponseRow key={r.id} response={r} mailboxes={mailboxes} />)}
                        </CardContent>
                    </Card>
                )}
            </div>
