...
export default function SlaSettings({ policies }: Props) {
    ...
    function updateTime(index: number, field: 'first_response_minutes' | 'resolution_minutes', part: 'hours' | 'mins', value: string) {
        ...
        const newMinutes = part === 'hours'
            ? hoursMinutesToMinutes(num, current.mins)
            : hoursMinutesToMinutes(current.hours, num)
                ...
                <form onSubmit={submit} className="space-y-4">
                    <div className="rounded-xl border border-border bg-card overflow-hidden">
                        ...
                        <div className="grid grid-cols-[160px_1fr_1fr_80px] gap-4 px-6 py-3 bg-muted/40 border-b border-border text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                            <span>Priority</span>
                            <span className="flex items-center gap-1.5"><ClockIcon className="h-3 w-3" />First Response</span>
                            <span className="flex items-center gap-1.5"><ClockIcon className="h-3 w-3" />Resolution</span>
                            <span>Active</span>
                        </div>
                        ...
                        {data.policies.map((policy, i) => {
                            ...
                            return (
                                ...
                                >
                                    <span className={`inline-flex w-fit items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${PRIORITY_COLORS[policy.priority]}`}>
                                    ...
                                    <Switch
                                        ...
                                        onCheckedChange={v => updatePolicy(i, 'active', v)}
                                    />
                                </div>
                            )
                        })}
                    ...
                    {errors.policies && (
                        <p className="text-xs text-destructive">{errors.policies}</p>
                    )}
