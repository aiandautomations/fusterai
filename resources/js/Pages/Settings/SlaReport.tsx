...
export default function SlaReport({
    days, total, fr_breached, res_breached, fr_achieved, res_achieved,
    fr_breach_rate, res_breach_rate, avg_first_response_min, avg_resolution_min,
    by_mailbox, by_agent,
}: Props) {
            ...
            <div className="w-full px-6 py-8 space-y-8">
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-1">
                            Service level agreement performance over the last {days} days.
                        </p>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                    ...
                    <StatCard
                        ...
                        label="Breach Rates"
                        value={<span><Breach rate={fr_breach_rate} /> / <Breach rate={res_breach_rate} /></span>}
                ...
                {by_agent.length > 0 && (
                    <section className="space-y-3">
                        ...
                        <div className="rounded-xl border border-border bg-card overflow-hidden">
                            <Table
                                headers={['Agent', 'Total', 'FR Breached', 'Res Breached', 'FR Breach Rate', 'Res Breach Rate', 'Avg FR Time']}
                                rows={by_agent.map(r => [
                                    r.agent_name,
                                    r.total,
                                    r.fr_breached,
        ...
        <table className="w-full text-sm">
            <thead>
                <tr className="border-b border-border bg-muted/40">
                    {headers.map(h => (
                        <th key={h} className="px-4 py-2.5 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide first:pl-6 last:pr-6">
                            {h}
                        </th>
                    ))}
                </tr>
