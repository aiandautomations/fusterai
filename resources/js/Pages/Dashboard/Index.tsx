...
interface AgentStat       { id: number; name: string; email: string; avatar: string | null; resolved_count: number }
interface RecentConv      { id: number; subject: string; status: string; priority: string; last_reply_at: string | null; customer: { id: number; name: string; email: string } | null }
...
function StatCard({ label, value, tone, icon }: { label: string; value: number; tone?: 'info' | 'warning' | 'primary' | 'default'; icon: React.ReactNode }) {
    const valueClass = tone === 'info' ? 'text-info' : tone === 'warning' ? 'text-warning' : tone === 'primary' ? 'text-primary' : 'text-foreground'
    return (
            ...
            <div className="w-full px-6 py-8 space-y-10">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <section className="lg:col-span-2 space-y-3">
                        <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-[0.12em]">New Conversations — Last 14 Days</h2>
                            ...
                            {trendData.length > 1 ? (
                                <ChartContainer config={trendConfig} className="h-48 w-full">
                                    <AreaChart data={trendData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                                        ...
                                        <Area type="monotone" dataKey="count" stroke="var(--color-count)" strokeWidth={2} fill="url(#dash-grad)" dot={false} activeDot={{ r: 4, fill: 'var(--color-count)' }} />
                                    </AreaChart>
                                </ChartContainer>
                            ) : (
                    ...
                    <div className="rounded-xl border border-border/80 bg-card/75 overflow-hidden">
                        {recent.length === 0 ? (
                            ...
                            <ul className="divide-y">
                                {recent.map(conv => (
                                    <li key={conv.id}>
                                        <Link href={`/conversations/${conv.id}`} className="flex items-center gap-4 px-5 py-3 hover:bg-muted/40 transition-colors">
                                            <div className="flex-1 min-w-0">
                                                <p className="text-sm font-medium truncate">{conv.subject || '(No subject)'}</p>
                                                <p className="text-xs text-muted-foreground truncate">{conv.customer?.name ?? conv.customer?.email ?? 'Unknown'}</p>
                                            </div>
                                            <span className={cn('text-[11px] font-medium px-2 py-0.5 rounded-full capitalize shrink-0', statusColors[conv.status] ?? 'bg-muted text-muted-foreground')}>
                                                {conv.status}
                                            </span>
                                            <span className="text-xs text-muted-foreground shrink-0 w-16 text-right">{timeAgo(conv.last_reply_at)}</span>
                                        </Link>
                                    </li>
                                ))}
                            </ul>
