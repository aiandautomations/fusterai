    ...
    return (
        <div className="flex h-full bg-muted/40 md:p-3 gap-0">
                    ...
                    <div className="relative z-10 px-3 py-2.5 border-b border-sidebar-border/70">
                        <Select value={activeMailbox} onValueChange={switchMailbox}>
                            ...
                            <SelectContent>
                                ...
                                {mailboxes?.map((mb) => (
                                    <SelectItem key={mb.id} value={String(mb.id)}>{mb.name}</SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                    </div>
                        ...
                        <>
                            <div className="space-y-1">
                                <button type="button" onClick={() => setInboxOpen((v) => !v)} className={sectionHeaderClass()}>
                                    {inboxOpen ? <ChevronDownIcon className="h-3.5 w-3.5" /> : <ChevronRightIcon className="h-3.5 w-3.5" />}
                                    <span className="flex-1">Inbox</span>
                                    {(counts?.open ?? 0) > 0 && <span className="text-[10px] text-sidebar-foreground/65">{counts?.open}</span>}
                                </button>
                                {inboxOpen && (
                            ...
                            <div className="mt-1 space-y-1">
                                <button type="button" onClick={() => setTagsOpen((v) => !v)} className={sectionHeaderClass()}>
                                    {tagsOpen ? <ChevronDownIcon className="h-3.5 w-3.5" /> : <ChevronRightIcon className="h-3.5 w-3.5" />}
                            ...
                            <div className="mt-1 space-y-1">
                                ...
                                >
                                    {foldersOpen ? <ChevronDownIcon className="h-3.5 w-3.5" /> : <ChevronRightIcon className="h-3.5 w-3.5" />}
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    {(folder.open_count ?? 0) > 0 && (
                                                        <span className="text-xs text-sidebar-foreground/45">{folder.open_count}</span>
                                                    )}
                                                </Link>
                                            );
                                        }) : (
            ...
            <div className={cn('flex-1 flex flex-col min-w-0 bg-background', fullHeight ? 'overflow-hidden' : 'overflow-y-auto')}>
                <header className="sticky top-0 z-20 flex items-center border-b border-border/60 bg-background/95 h-16 px-4 md:px-5 backdrop-blur-sm shrink-0">
                    <div className="flex w-full items-center justify-between gap-4">
                        ...
                        <nav className="hidden items-center gap-0.5 lg:flex">
                            ...
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="start" className="w-52 border-0 bg-popover/95 shadow-xl backdrop-blur">
                                    ...
                                    <DropdownMenuGroup>
                                        ...
                                        <DropdownMenuItem asChild>
                                            <Link href="/settings/folders" className={cn('w-full', path.startsWith('/settings/folders') && 'text-primary')}>
                                                <FolderIcon className="h-4 w-4" /><span>Folders</span>
                                            </Link>
                                        </DropdownMenuItem>
                                        <DropdownMenuItem asChild>
                                            <Link href="/settings/canned-responses" className={cn('w-full', path.startsWith('/settings/canned-responses') && 'text-primary')}>
                                                <MessageSquareTextIcon className="h-4 w-4" /><span>Canned Responses</span>
                                            </Link>
                                        </DropdownMenuItem>
                                    </DropdownMenuGroup>
                                </DropdownMenuContent>
                            ...
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <button className={cn('inline-flex h-8 items-center gap-1.5 rounded-lg px-3 text-[13px] font-medium whitespace-nowrap transition-all', aiNavActive ? 'text-primary font-semibold' : 'text-muted-foreground hover:bg-muted/50 hover:text-foreground')}>
                                ...
                                <DropdownMenuContent align="start" className="w-52 border-0 bg-popover/95 shadow-xl backdrop-blur">
                                    <DropdownMenuGroup>
                                        <DropdownMenuItem asChild>
                                            <Link href="/settings/ai" className={cn('w-full', path.startsWith('/settings/ai') && 'text-primary')}>
                                                <BrainIcon className="h-4 w-4" /><span>AI Config</span>
                                            </Link>
                                        </DropdownMenuItem>
                                        <DropdownMenuItem asChild>
                                            <Link href="/ai/knowledge-base" className={cn('w-full', path.startsWith('/ai/') && 'text-primary')}>
                                                <BookOpenIcon className="h-4 w-4" /><span>Knowledge Base</span>
                                            </Link>
                                        </DropdownMenuItem>
                                        <DropdownMenuItem asChild>
                                            <Link href="/automation" className={cn('w-full', path.startsWith('/automation') && 'text-primary')}>
                                                <BotIcon className="h-4 w-4" /><span>Automation</span>
                                            </Link>
                                        </DropdownMenuItem>
                                    </DropdownMenuGroup>
                                </DropdownMenuContent>
                            ...
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <button className={cn('inline-flex h-8 items-center gap-1.5 rounded-lg px-3 text-[13px] font-medium whitespace-nowrap transition-all', customNavActive ? 'text-primary font-semibold' : 'text-muted-foreground hover:bg-muted/50 hover:text-foreground')}>
                                ...
                                <DropdownMenuContent align="start" className="w-52 border-0 bg-popover/95 shadow-xl backdrop-blur">
                                    <DropdownMenuGroup>
                                        <DropdownMenuItem asChild>
                                            <Link href="/settings/appearance" className={cn('w-full', path.startsWith('/settings/appearance') && 'text-primary')}>
                                                <MonitorIcon className="h-4 w-4" /><span>Appearance</span>
                                            </Link>
                                        </DropdownMenuItem>
                                        <DropdownMenuItem asChild>
                                            <Link href="/settings/live-chat" className={cn('w-full', path.startsWith('/settings/live-chat') && 'text-primary')}>
                                                <MessageSquareIcon className="h-4 w-4" /><span>Live Chat Widget</span>
                                            </Link>
                                        </DropdownMenuItem>
                                    </DropdownMenuGroup>
                                </DropdownMenuContent>
                            ...
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <button className={cn('inline-flex h-8 items-center gap-1.5 rounded-lg px-3 text-[13px] font-medium whitespace-nowrap transition-all', settingsNavActive ? 'text-primary font-semibold' : 'text-muted-foreground hover:bg-muted/50 hover:text-foreground')}>
                                    ...
                                    <DropdownMenuGroup>
                                        <DropdownMenuItem asChild>
                                            <Link href="/settings" className={cn('w-full', path === '/settings' && 'text-primary')}>
                                                <SettingsIcon className="h-4 w-4" /><span>General</span>
                                            </Link>
                                        </DropdownMenuItem>
                                        <DropdownMenuItem asChild>
                                            <Link href="/settings/users" className={cn('w-full', path.startsWith('/settings/users') && 'text-primary')}>
                                                <UsersIcon className="h-4 w-4" /><span>Team</span>
                                            </Link>
                                        </DropdownMenuItem>
                                        <DropdownMenuItem asChild>
                                            <Link href="/settings/modules" className={cn('w-full', path.startsWith('/settings/modules') && 'text-primary')}>
                ...
                <div className={cn('min-h-0 flex-1', fullHeight ? 'flex overflow-hidden' : '')}>
                    {children}
                </div>
            </div>
            </div>
